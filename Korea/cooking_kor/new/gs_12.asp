<html>
<head>
	<meta http-equiv=Content-Type content="text/html; charset=windows-1251">
	<title>Tour2Korea.com: О корейской кухне</title>
	<LINK REL="StyleSheet" HREF="t2k.css" type="text/css">
	<script language="JavaScript" src="t2k.js"></script>
	</script>
</head>	
<body topmargin="0" leftmargin="0" marginheight="0" marginwidth="0" bgcolor="#FFFFFF">
				<table cellpadding="0" cellspacing="0" border="0" width="544">
					<tr>
						<td height="20" valign="top"><img src="images/title_cls.gif" border="0"></td>
						<td align="right"><a href="cls_main.asp"><img src="/common/images/btn_citis_list.gif" border="0"></a></td>
					</tr>
					<tr><td background="/common/images/dot_01.gif" colspan="2"><img src="/common/images/trans.gif" width="1" height="1" border="0"></td></tr>
					<tr bgcolor="#F5F5F5">
						<td height="25" style="padding-left:10px"><b><a href="cls_gs.asp" class="c">Гёнсан-до</a><span class="c">,</span> Агу-ёрим</b></td>
						<td align="right">
						<!-- select -->
						<select name="s02" onChange="window.open(this.options[this.selectedIndex].value,'_self')">
							<option value="#" selected>Р’С‹Р±РѕСЂ Р±Р»СЋРґР°</option>
							<option value="gs_03.asp">РњСѓР±Р°Рї Рё Р”СЌРіСѓ-С‚С…Р°РЅ</option>
							<option value="gs_04.asp">РҐРѕС‚С‡Р¶РµСЃР°-Р±Р°Рї</option>
							<option value="gs_11.asp">РђРіСѓ-С‚РёРј</option>
							<option value="gs_13.asp">РњРёРґРѕРґРѕРє-С‚РёРј</option>
							<option value="gs_14.asp">Р”РѕРЅРЅСЌ-РїС…Р°С‡Р¶РѕРЅ</option>
							<option value="gs_16.asp">РљРёРјС‡Рё</option>
						</select>
						<!-- // select --></td>
					</tr>
					<tr><td><img src="/common/images/trans.gif" width="1" height="20" border="0"></td></tr>
				</table>
				<!-- Explain -->
				<table cellpadding="0" cellspacing="0" border="0" width="544">
					<tr>
						<td>
						<table cellpadding="0" cellspacing="0" border="0" align="left"> 
							<tr>
								<td><img src="images/gs12.jpg" border="0" width="174" height="125" onclick="javascript:pop('popup.asp?img_name=gs12',550,500,0)" style="cursor:hand"></td>
								<td width="10"><img src="/common/images/trans.gif" width="10" height="1" border="0"></td>
							</tr>
							<tr><td><img src="/common/images/trans.gif" width="1" height="7" border="0"></td></tr>
						</table>
						<b>Агу-ёрим (тушёный морской чёрт - англер) (приложение)</b><br><br>
						Подготовоенные кусочки англера тушить с добавлением соевого соуса, имбирного сока и сахара.</td>
					</tr>								
					<tr><td><img src="/common/images/trans.gif" width="1" height="10" border="0"></td></tr>
				</table>
				<table cellpadding="0" cellspacing="0" border="0" width="544">
					<tr valign="top">
						<td width="10" class="o">*</td>
						<td class="o">Мясо морского чёрта (англера) довольно жёсткое и требует продолжительного тушения. </td>
					</tr>
					<tr><td><img src="/common/images/trans.gif" width="1" height="10" border="0"></td></tr>
					<tr><td align="right" colspan="2"><a href="#"><img src="/common/images/top.gif" border="0"></a></td></tr>
					<tr><td><img src="/common/images/trans.gif" width="1" height="20" border="0"></td></tr>
					<tr><td height="1" bgcolor="#E6E6E7" colspan="2"><img src="/common/images/trans.gif" width="1" height="1" border="0"></td></tr>
				</table>
</body>
</html>